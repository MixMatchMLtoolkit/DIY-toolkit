...
void loop() {
  ...
  if (micros() - timer >= (1000000 / READ_RATE) ) { //Timer reduce number of reading per second
    ...
    else if (!mfrc522.PICC_IsNewCardPresent() && id != '0' && !checkEmpty) {
      id = '0';
      checkEmpty = true;
    }
    Serial.write(id);
...
byte readID() {
  ...
  if (status != MFRC522::STATUS_OK) {
    ...
    if (status == 7) { // status code 7 means the crc_a didn't match --> error can occasionally occur when removing the token
      mfrc522.PCD_Reset(); //reset and init to continue reading
      mfrc522.PCD_Init();
    }
    // return;
  }
  ...
  //delay(50); //1000 change value if you want to read cards faster
